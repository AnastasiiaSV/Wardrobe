...
class Cropper {
   ...
   * });
    ...
    constructor(options) {
        ...
        this.limit = options.limit || 550; // default to 550px
        // Create the cropping square with the handle's size
        this.crop = {
            size: { x: options.size.width/2, y: options.size.height/2 },
            pos: { x: 1, y: 1 },
            handleSize: 10
        };
    ...
    export(img) {
       ...
       // img.setAttribute('src', this.previewCanvas.toDataURL());
        //img.setAttribute('value', this.previewCanvas.toDataURL());
        img.value = this.previewCanvas.toDataURL();
        //alert(this.previewCanvas.toDataURL());
    }
    ...
    drawCropWindow() {
        ...
        this.c.strokeStyle = 'rgba(170, 0, 3, 0.51)';
        this.c.fillStyle = 'rgba(170, 0, 3, 0.51)';
...
var dimensions = { width: 550, height: 550 }; ////////////// Было 128/128
...
try {
    ...
    if (uploader && editor) {
        ...
        //document.getElementById('id_editorcanvas').className +=" canvas"
